...
pub struct TrackState<'m> {
    trk_number: usize,
    trk_iter: std::slice::Iter<'m, midly::TrackEvent<'m>>,
    time: u32,
    evt: Option<&'m midly::TrackEvent<'m>>,
}
impl<'m> TrackState<'m> {
    fn new(trk_number: usize, trk_iter: std::slice::Iter<'m, midly::TrackEvent<'m>>) -> TrackState<'m> {
        TrackState {
            trk_number,
            trk_iter,
            time: 0,
        ...
        match (self.evt.is_some(), other.evt.is_some()) {
            ...
            (true, true) => match self.sort_key().cmp(&other.sort_key()) {
                Ordering::Equal => match self.evt.as_ref().unwrap().kind {
                    midly::TrackEventKind::Meta(_) => Ordering::Less,
                    _ => match other.evt.as_ref().unwrap().kind {
                        midly::TrackEventKind::Meta(_) => Ordering::Greater,
                        _ => Ordering::Equal,
                    },
                },
                o => o,
...
#[derive(Debug)]
pub struct MidiTimedEvent<'m>(pub u64, pub usize, pub &'m midly::TrackEventKind<'m>);
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            if let Some(evt) = opt_evt {
                return Some(MidiTimedEvent(time as u64, trk_number, &evt.kind));
            }
        }
    }
}
...
impl<'m> Iterator for MidiTimedIterator<'m> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            if let Some(MidiTimedEvent(time, trk, evt_kind)) = opt_tuple {
                ...
                match evt_kind {
                    &midly::TrackEventKind::Meta(midly::MetaMessage::Tempo(tmp)) => {
                        self.update_timebase(tmp.as_int());
                    }
                    _ => {
                        return Some(MidiTimedEvent(self.current_time_us, trk, evt_kind));
                    }
                }
            } else {
    ...
    for i in 0..container.nr_of_tracks() {
        ...
        for evt in container.iter().filter(|e| e.1 == i) {
            match evt.2 {
                ...
                midly::TrackEventKind::Meta(mm) => match mm {
...
#[cfg(test)]
mod tests {
    use generic::midi_container::{self, MidiTimedEvent};

    #[test]
    fn test_01() {
        let midi_fname = "data/Marche_aux_Flambeaux.mid";
    ...
    fn test_02() {
        ...
        assert_eq!(
            container
                .iter()
                .filter(|MidiTimedEvent(_time, track_id, _evt)| *track_id == 0)
                .count(),
            6
        );
    }
    ...
    fn test_03() {
        ...
        assert_eq!(
            container
                .iter()
                .filter(|MidiTimedEvent(_time, track_id, _evt)| *track_id == 1)
                .count(),
            1679
        );
    }
    ...
    fn test_04() {
        ...
        assert_eq!(
            container
                .iter()
                .filter(|MidiTimedEvent(_time, track_id, _evt)| *track_id == 2)
                .count(),
            2423 - 6 - 1679
        );
    }
    ...
    fn test_06() {
        ...
        let container = midi_container::MidiContainer::from_buf(&smf_buf).unwrap();
        let mut last_time = 0;
        for MidiTimedEvent(time, _, _) in container.iter() {
    ...
    fn test_16() {
        ...
        for MidiTimedEvent(time_us, _, _) in container.iter().timed(&container.header().timing) {
